...
st.title("Bytehash Labs Assignment")
st.write("Extract structured data from websites or PDFs")
...
def extract_structured_data_with_gemini(text, source_name):
    ...
    You are an expert data extractor. Given the following raw content (from a website or PDF), identify and extract all credit card details in a structured JSON format.

Focus only on credit card information. Ignore unrelated content like articles, footers, disclaimers, etc.

For each credit card, extract and group details into this format:
- card_name
- issuing_bank
- joining_fee
- annual_fee
- reward_structure (e.g., points, cashback, categories, limits)
- cashback_offers (if applicable)
- interest_rate
- credit_limit (if mentioned)
- eligibility_criteria (e.g., income requirements, employment)
- other_features (like lounge access, insurance, etc.)

Some fields may be missing in some cards—leave them empty or omit them and always give full respomse dont truncate.
    """
    try:
